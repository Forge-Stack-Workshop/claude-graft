 ...
 *
 * Configuration precedence: env var > hooks-config.json → circuitBreaker > default.
 *   failureThreshold / CB_FAILURE_THRESHOLD   - failures before opening (default: 3)
 *   resetTimeoutMs   / CB_RESET_TIMEOUT_MS     - ms before half-open trial (default: 60000)
 *   stateFile        / CB_STATE_FILE           - path to state file (config path is
...
// ── Configuration (hooks-config.json → circuitBreaker, overridable by env) ──────
const ROOT = path.resolve(__dirname, "../../");
const CONFIG_PATH = path.join(ROOT, ".claude", "config", "hooks-config.json");

function loadConfig() {
  try {
    return JSON.parse(fs.readFileSync(CONFIG_PATH, "utf8")).circuitBreaker ?? {};
  } catch {
    return {};
  }
}
...
const FAILURE_THRESHOLD = parseInt(
  process.env.CB_FAILURE_THRESHOLD ?? CFG.failureThreshold ?? "3",
  10
);
const RESET_TIMEOUT_MS = parseInt(
  process.env.CB_RESET_TIMEOUT_MS ?? CFG.resetTimeoutMs ?? "60000",
  10
);
const STATE_FILE =
  process.env.CB_STATE_FILE ??
  (CFG.stateFile
    ? path.resolve(ROOT, CFG.stateFile)
