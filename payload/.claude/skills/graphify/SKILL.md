...
Requires the **graphify** CLI. Verify before use; warn if missing:

```bash
command -v graphify >/dev/null 2>&1 || echo "WARN: graphify not installed — install the graphify CLI first; without it, fall back to reading files / grep for the query"
```

If `graphify` is absent, do not fail the task — answer from the existing `graphify-out/`
artifacts if present, otherwise fall back to normal file reading and search.
